...
// MARK: - Custom MTKView to handle input
...
final class Renderer: NSObject, MTKViewDelegate {
    ...
    init?(mtkView: MTKView) {
        ...
        stateTextures = [
            makeStateTexture(initial: true),
            makeStateTexture(initial: false)
        ]
    }
    ...
    func makeStateTexture(initial: Bool) -> MTLTexture {
        ...
        guard let tex = device.makeTexture(descriptor: desc) else {
            fatalError("Failed to create state texture")
        }
    ...
    func draw(in view: MTKView) {
        ...
        if shouldStep {
            ...
            if let encoder = commandBuffer.makeComputeCommandEncoder() {
                ...
                encoder.setBytes(&grid,
                                 length: MemoryLayout<GridSize>.stride,
                                 index: 0)
...
final class AppDelegate: NSObject, NSApplicationDelegate {
    var window: NSWindow!
    var metalView: GameView!
    var renderer: Renderer!

    func applicationDidFinishLaunching(_ notification: Notification) {
        let size = NSSize(
            width: CGFloat(WIDTH) * PIXEL_SCALE,
            height: CGFloat(HEIGHT) * PIXEL_SCALE
        )
        ...
        metalView = GameView(frame: NSRect(origin: .zero, size: size))
        ...
        // Hook Space key to toggle pause
        metalView.onSpace = { [weak self] in
